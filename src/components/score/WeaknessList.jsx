...
import { getScoreBand } from '../../utils/scoreDisplay';

function resolveBarColor(score, maxScore) {
...
function resolveDotColor(barColor) {
  if (barColor === 'success') return 'var(--color-leaf)';
  if (barColor === 'warning') return 'var(--color-amber-400)';
...
export default function WeaknessList({ categories = [] }) {
  return (
    <ul className="flex flex-col gap-4 list-none p-0 m-0">
      {categories.map(({ name, score, maxScore }, index) => {
        const pct = maxScore > 0 ? Math.round((score / maxScore) * 100) : 0;
        const barColor = resolveBarColor(score, maxScore);
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                <span
                  ...
                  style={{ backgroundColor: resolveDotColor(barColor) }}
                  aria-hidden="true"
                />
